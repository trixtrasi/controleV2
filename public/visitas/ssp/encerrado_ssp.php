...
$columns = array(
    array('db' => '`visitantes`.`visitantes__id`', 'dt' => 0, 'field' => 'visitantes__id', 'formatter' => function ($d, $row) {
        return '<button data-acao="visualizar" data-visitante-id="' . $d . '" data-visita-id="' . $row[5] . '" type="button" class="btn btn-sm btn-link col-12"  onClick="visualizarModal(this)"><img src="' . $row[8] . '" alt="visualizar" width="50" height="50"></button>';
    }),
    ...
    array('db' => '`visitantes__visitas`.`visitas__saiu_d_h`', 'dt' => 4, 'field' => 'visitas__saiu_d_h', 'formatter' => function ($d) {
        return formatterDate($d);
    }),
    array('db' => '`visitantes__visitas`.`visitas__id`', 'dt' => 5, 'field' =>
        'visitas__id', 'formatter' => function ($d) use ($nivel_user) {
          if ($nivel_user == 5){
              return '<button data-acao="excluirVisita" data-id="' . $d . '" type="button" class="btn btn-sm btn-danger col-12"  onClick="return confirmSubmit(this)">excluir visita</button>';
          }else{
...
$extraWhere = "`visitantes__visitas`.`visitas__status` = 0";
$groupBy = null;
$having = null;
